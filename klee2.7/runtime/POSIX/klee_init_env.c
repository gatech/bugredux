...
static char *__get_sym_str_detail(int numChars, char *name, char* detail_file_name) {
  int i;
  char *s = malloc(numChars+1);
  klee_mark_global(s);
  klee_make_symbolic(s, numChars+1, name);
  ...
  //open detail_file_name
  FILE *detail_file = fopen ( detail_file_name, "r" );

  if (detail_file!=NULL) {
	  char line[1024]; //max line size is 1024
	  while (fgets ( line, sizeof line, detail_file ) != NULL) {
		  char *posc = strchr (line, '#');
		  int pos = posc ? posc - line : -1;
		  if (pos==-1)
			  continue;
		  char *indexstr = (char*) malloc(pos+1);
		  strncpy(indexstr,line,pos);
		  indexstr[pos]='\0';
		  unsigned index = __str_to_int(indexstr,"detail file contains invalid index");
		  free(indexstr);
		  if (index<numChars) {
			  s[index] = line[pos+1]; // only consider the char after "#"
			  printf("arr index %d set to %c\n",index,line[pos+1]);
		  }
	  }
	  fclose(detail_file);
  }
...
usage: (klee_init_env) [options] [program arguments]\n\
  ...
  while (k < argc) {
    ...
    //added by wei
    else if (__streq(argv[k], "--sym-arg-detail") || __streq(argv[k], "-sym-arg-detail")) {
      const char *msg = "--sym-arg expects an integer argument <max-len>";
      if (++k == argc)
	__emit_error(msg);
      ...
      char* detail_file = argv[k++];

      sym_arg_name[3] = '0' + sym_arg_num++;
      __add_arg(&new_argc, new_argv,
                __get_sym_str_detail(max_len, sym_arg_name,detail_file),
                1024);
    }
