...
namespace {
  ...
  cl::opt<bool>
  UseReplayPathExecutor("use-path-replay-executor");
...
//extern cl::opt<bool> UseCallSeqReplayNSSearch;
...
void Executor::run(ExecutionState &initialState) {
  ...
  while (!states.empty() && !haltExecution) {
    ...
    // add a check for shortestpath searcher
  //  if (isa<ShortestPathSearcher>(searcher)) {
    	if (searcher->empty()){
  ...
  //  }
    ...
    if (usenonstopsearcher) {
    	if (((CallSeqReplayNonStopSearcher*)searcher)->getTarget) {
    		terminateStateEarly(state, "execution halting by searcher");
    		std::cerr<<"terminate a state\n";
    		removecurstate = true;
    	}
    }
