...
class PCLoggingSolver : public SolverImpl {
  ...
  void startQuery(const Query& query, const char *typeName,
    ...
    std::ofstream os2("singlequery.txt");

    os << "# Query " << queryCount++ << " -- "
       << "Type: " << typeName << ", "
       << "Instructions: " << instructions << "\n";

    os2 << "# Query " << queryCount++ << " -- "
       << "Type: " << typeName << ", "
       << "Instructions: " << instructions << "\n";
    ...
    os << "Finished print Query\n";

    printer->printQuery(os2, query.constraints, query.expr,
                        evalExprsBegin, evalExprsEnd,
                        evalArraysBegin, evalArraysEnd);
    os2 << "Finished print Query\n";
